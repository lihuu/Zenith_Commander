...
struct FileRowView: View, Equatable {
    ...
    static func == (lhs: FileRowView, rhs: FileRowView) -> Bool {
        lhs.file.id == rhs.file.id && lhs.file.name == rhs.file.name
            && lhs.file.gitStatus == rhs.file.gitStatus
            && lhs.isActive == rhs.isActive && lhs.isSelected == rhs.isSelected
            && lhs.isPaneActive == rhs.isPaneActive
            && lhs.rowIndex == rhs.rowIndex && lhs.isEditing == rhs.isEditing
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 4)
                .stroke(
                    isDropTarget ? Theme.accent : cursorBorderColor,
                    lineWidth: isDropTarget
                        ? 2 : (isActive && !isPaneActive ? 1 : 0)
                )
                .padding(1)
        )
